...
def cost_path(linkAction):
    distance = linkAction.distance
    speed = info.SPEED_RANGES[linkAction.highway_type][1] * 1000
...
class Roads(dict):
    ...
    def link_speed_history(self, link, time=0):
        ...
        return int(
            top / tools.generate_slowdown_multiplier(link.distance, top, self.base_traffic[time], *(link.link_params),
                                                     time=time))
    ...
    def realtime_link_speed(self, link, time=0):
        ...
        _delta_dist = tools.compute_distance(self.mean_lat_lon[0], self.mean_lat_lon[1], self[link.source].lat,
                                             self[link.source].lon)
